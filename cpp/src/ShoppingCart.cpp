    ...
    for (const auto& productQuantity : productQuantities) {
        ...
        if (offers.find(product) != offers.end()) {
            ...
            if (offer.getOfferType() == SpecialOfferType::ThreeForTwo) {
                ...
                boughtProductsNum = 2;
                if (quantityAsInt >= boughtProductsNum) {
                    double total = offer.getArgument() * (quantityAsInt / boughtProductsNum) + quantityAsInt % boughtProductsNum * unitPrice;
            ...
            if (offer.getOfferType() == SpecialOfferType::ThreeForTwo && quantityAsInt > boughtProductsNum) {
                double discountAmount = quantity * unitPrice - ((boughtProductsRatio * boughtProductsNum * unitPrice) + quantityAsInt % 3 * unitPrice);
            ...
            if (offer.getOfferType() == SpecialOfferType::FiveForAmount && quantityAsInt >= boughtProductsNum) {
                double discountTotal = unitPrice * quantity - (offer.getArgument() * boughtProductsRatio + quantityAsInt % boughtProductsNum * unitPrice);
